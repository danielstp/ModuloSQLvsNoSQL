...
class Category(models.Model):
    ...
    def countFilms(self):
        return FilmCategory.objects.filter(category_id=self.category_id).count()
...
class FilmActor(models.Model):
    actor = models.ForeignKey(
        Actor, on_delete=models.RESTRICT
...
class FilmCategory(models.Model):
    film = models.OneToOneField(
        Film, on_delete=models.RESTRICT, primary_key=True, related_name="film_category"
    )  # The composite primary key (film_id, category_id) found, that is not supported. The first column is selected.
    category = models.ForeignKey(
        Category, on_delete=models.RESTRICT, related_name="category_film"
    )
    last_update = models.DateField(db_index=True, auto_now=True)
...
class Payment(models.Model):
    ...
    rental = models.ForeignKey(
        "Rental", on_delete=models.RESTRICT, blank=True, null=True
    )
...
class Store(models.Model):
    ...
    manager_staff = models.ForeignKey(
        Staff, on_delete=models.RESTRICT, related_name="manager_staff_1"
    )
    address = models.ForeignKey(Address, on_delete=models.RESTRICT)
    last_update = models.DateField(db_index=True, auto_now=True)
